...
function _processConfirm (message,resultCallback,title,buttonLabels) {
    ...
    message = message || "";
    title = title || "Confirm";
    var processBtns = function () {
        if(typeof(buttonLabels) === 'string')
            return (!buttonLabels || 0 === buttonLabels.length) ? [] : buttonLabels.split(',');
        if(buttonLabels.constructor.toString().indexOf('Array') > -1)
            return buttonLabels;
    };
    var btnArray = processBtns();
    var buttons = [];
